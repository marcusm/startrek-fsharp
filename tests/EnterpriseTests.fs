...
module ``Enterprise Tests`` =
    ...
    type ``Given a damaged enterprise``() =
        ...
        [<Fact>]
        member x.`` A heavily damaged enterprise is not completely repaired ``() =
            let random = Mock<IRandomService>()

            random.Setup(fun r -> r.NextDouble()).Returns(0.6)
            |> ignore
        ...
        [<Fact>]
        member x.`` An improved enterprise will not be harmed``() =
            let improved =
                [ { Amount = -4; System = PhotonTubes } ]
